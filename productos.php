...
if (!empty($busquedaProducto)) {
  ...
  $stmt = $conn->prepare("SELECT p.id, p.nombre, p.descripcion, p.precio, p.imagen, p.stock, p.sku, c.nombre AS categoria_nombre, c.id AS categoria_id FROM productos p 
  ...
  $stmt_count = $conn->prepare("SELECT COUNT(*) AS total FROM productos p WHERE p.activo = 1 AND p.categoria_id = ?");
  ...
  $mostrarPaginacion = ($total_paginas > 1);
} elseif ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['cat'])) {
  $categoria_id = intval($_POST['cat']);

  if ($categoria_id <= 0) {
    header("Location: 404.php");
    exit();
  }
  header("Location: ?cat=" . $categoria_id);
  exit();
} else {
  header("Location: 404.php");
  exit();
...
<body>
  <div class="min-h-screen bg-[#0b0b0a]">
    ...
    <?php if (isset($mostrarPaginacion) && $mostrarPaginacion): ?>
      <div id="paginado-productos" class="flex gap-6 flex-wrap justify-center mt-4 mb-8">
        <?php for ($i = 1; $i <= $total_paginas; $i++):
          $color = ($i == $pagina_actual) ? "bg-yellow-400 text-gray-900" : "bg-gray-200 hover:bg-[#272726] hover:text-white text-black";
          $url = "?cat=" . $categoria_id . "&page=" . $i;
        ?>
          <a class='px-5 py-3 font-bold rounded-lg <?php echo $color; ?>' href='<?php echo $url; ?>'>
            <?php echo $i; ?>
          </a>
        <?php endfor; ?>
      </div>
