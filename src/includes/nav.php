  ...
  <div class="lg:mx-65 flex items-center justify-between relative">
    ...
    <div class="w-full mx-auto flex items-center justify-between hidden lg:flex">
      ...
      <div class="flex items-center justify-end gap-4 flex-shrink-0 w-1/4">
        <div class="flex items-center justify-end flex-shrink-0 group">
          <?php if ($estaLogueado): ?>
            <div class="hidden -mt-1 xl:flex flex-col items-end text-right text-xs leading-none justify-center">
              <a class="text-gray-400 cursor-default" href="#">Bienvenido</a>
              <a href="#" class="text-white font-medium hover:text-gray-300 transition-colors duration-200 whitespace-nowrap">
                <?php echo htmlspecialchars($user_display); ?>
              </a>
              <div class="absolute top-full right-30 mt-2 w-48 bg-[#010001] rounded-lg shadow-xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-50 ">
                <div class="py-2">
                  <form action="<?php echo INCLUDES_URL . 'logout.php'; ?>" method="POST">
                    <button type="submit" class="w-full text-center px-4 py-2 text-sm text-gray-300  hover:text-red-400 transition-colors duration-150" style="background:none;border:none;cursor:pointer;font:inherit;">
                      Cerrar Sesion
                    </button>
                  </form>
                </div>
          ...
          <?php
          if ($estaLogueado) { ?>
            <a href="#" class="text-white hover:text-gray-300 transition-colors duration-200 p-1 hidden lg:block">
              <div class="flex flex-row">
                </svg>
                <svg class="w-4 h-4 inline-block m-1 mt-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <?php
            ...
            <a href="<?php echo BASE_URL . 'cuenta.php'; ?>" class="text-white hover:text-gray-300 transition-colors duration-200 p-1 hidden lg:block">
              <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                <path stroke-linecap="round" stroke-linejoin="round" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />
              </svg>
            </a><?php
        ...
        <?php if ($esAdmin): ?>
          <a href="<?php echo ADMIN_URL . 'admin-panel.php'; ?>"
            class="hidden lg:block text-white bg-red-600 hover:bg-red-700 text-xs ml-1 font-bold py-1 px-3 rounded-full duration-200 transition active:scale-95">
            ADMIN
          </a>
        <?php endif; ?>
      </div>
...
<div class="w-full <?php echo $esAdmin ? 'bg-black' : 'bg-black'; ?> bg-opacity-90 backdrop-filter backdrop-blur-md hidden lg:block px-4 py-1.5 border-t <?php echo $esAdmin ? 'border-gray-800' : 'border-gray-800'; ?> ">
  <div class="flex gap-8 text-sm text-gray-400 max-w-7xl mx-auto justify-center">
    <ul class="flex gap-30 shine-effect font-normal items-center antialiased">
